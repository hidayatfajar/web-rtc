...
export const useMeetingStore = defineStore("meeting", {
  ...
  actions: {
    ...
    removeParticipant(socketId: string) {
      ...
      console.log(
        "[Store] Participants before removal:",
        this.participants.length,
      );
      this.participants = this.participants.filter((p) => p.id !== socketId);
      console.log(
        "[Store] Participants after removal:",
        this.participants.length,
      );
    },
    ...
    updateParticipantMedia(
      socketId: string,
      { isMuted, isVideoOff }: { isMuted?: boolean; isVideoOff?: boolean },
    ) {
    ...
    sendMessage(message: string) {
      ...
      console.warn(
        "[Store] sendMessage called but should use socket emit instead",
      );
    },
    ...
    clearChatHistory(roomId?: string) {
      ...
      if (typeof window !== "undefined" && targetRoomId) {
        ...
        console.log(
          "[Store] ✅ Cleared chat history from localStorage for room:",
          targetRoomId,
          "key:",
          key,
        );
        ...
        if (targetRoomId === this.roomId) {
          this.messages = [];
          console.log(
            "[Store] ✅ Cleared messages array. New count:",
            this.messages.length,
          );
        }
      } else {
        console.warn(
          "[Store] Cannot clear chat history - missing targetRoomId or not in browser",
        );
      }
    },
